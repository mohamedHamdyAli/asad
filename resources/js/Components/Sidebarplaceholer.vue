    ...
    <div class="flex-1 overflow-y-auto px-3 py-4 sidebar-scroll">
      <nav class="space-y-4">
        ...
        <div class="rounded-2xl bg-white/15 backdrop-blur-md border border-white/20 shadow-sm overflow-hidden">
          <button
            type="button"
            class="w-full px-3 pt-3 pb-2 flex items-center justify-between"
            @click="toggle('management')"
            :aria-expanded="open.management ? 'true' : 'false'"
          >
            ...
            <svg
              class="h-4 w-4 text-black/60 transition-transform duration-200"
              :class="open.management ? 'rotate-180' : 'rotate-0'"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
          ...
          <div v-show="open.management" class="pb-2">
            ...
            <NavItem
              ...
              :to="route('contractors-management')"
        ...
        <div class="rounded-2xl bg-white/15 backdrop-blur-md border border-white/20 shadow-sm overflow-hidden">
          <button
            ...
            class="w-full px-3 pt-3 pb-2 flex items-center justify-between"
            @click="toggle('app')"
            :aria-expanded="open.app ? 'true' : 'false'"
          >
            <span class="text-[11px] font-semibold tracking-widest uppercase text-black/70">
              App Content
            </span>
            ...
            <svg
              class="h-4 w-4 text-black/60 transition-transform duration-200"
              :class="open.app ? 'rotate-180' : 'rotate-0'"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
        ...
        <div class="rounded-2xl bg-white/15 backdrop-blur-md border border-white/20 shadow-sm overflow-hidden">
          <button
            ...
            class="w-full px-3 pt-3 pb-2 flex items-center justify-between"
            @click="toggle('system')"
            :aria-expanded="open.system ? 'true' : 'false'"
          >
            <span class="text-[11px] font-semibold tracking-widest uppercase text-black/70">
              System
            </span>
            ...
            <svg
              class="h-4 w-4 text-black/60 transition-transform duration-200"
              :class="open.system ? 'rotate-180' : 'rotate-0'"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
