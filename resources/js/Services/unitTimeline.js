...
/* ---------- API ---------- */
export const UnitTimelineApi = {
  list: async (unitId) => {
    const { data } = await axios.get(`/api/unit-timeline/${unitId}`);
    return unwrapList(data).map(normalize);
  },
  create: async (formData) => {
    const { data } = await axios.post(`/api/unit-timeline/create`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    return data;
  },
  update: async (formData) => {
    const { data } = await axios.post(`/api/unit-timeline/update`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    return data;
  },
  remove: async (id) => {
    const { data } = await axios.delete(`/api/unit-timeline/delete/${id}`);
    return data;
  },
};

/* ---------- FormData builders (align with UnitTimeLineRequest) ---------- */
// CREATE: items = [{ file:File, title:{en,ar} }]
export function buildTimelineCreateFD(unit_id, items = []) {
...
export function buildTimelineUpdateFD(unit_id, items = []) {
  const fd = new FormData();
  fd.append("unit_id", String(unit_id));
  items.forEach((it, i) => {
