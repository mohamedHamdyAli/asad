  ...
  <AuthenticatedLayout>
    <div class="p-6 space-y-6">
      <div class="flex items-center justify-between">
        <h2 class="text-2xl font-bold text-dash-title">Consultants</h2>
        <button class="px-3 py-1 border rounded text-black" @click="fetchList">Refresh</button>
      </div>

      <!-- Create one contractor -->
      <div class="bg-white p-4 rounded shadow">
        <h3 class="text-lg font-bold mb-3">Add Consultant</h3>
      ...
      <!-- List -->
      <div class="bg-white p-4 rounded shadow">
        <h3 class="text-lg font-bold mb-3">All Consultants</h3>
...
async function createOne(){
  ...
  createErr.value = ''
  try {
    const fd = buildConsultantsCreateFD([ createForm.value ])
    await ConsultantsApi.create(fd)
    await fetchList()
    // reset
...
async function saveOne(id){
  ...
  try {
    ...
    const fd = buildConsultantsUpdateFD(payload)
    await ConsultantsApi.update(id, fd)
